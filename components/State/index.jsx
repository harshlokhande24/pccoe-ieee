            ...
            <div className="bg-light-purple grid place-items-center grid-cols-1 gap-4 md:grid-cols-3 sm:grid-cols-2" >
                {
                    states.map((state, index) =>
                        <div title={state.title} key={index} className="state-card flex flex-col justify-center items-center py-8 px-12 text-center rounded-md" data-aos={state.aos}>
                            <span className="mb-2 text-white text-4xl">
                                {state.title == "Instagram" ? <FaInstagram /> : state.title == "Linkedin" ? <FaLinkedin /> : state.title == "Facebook" ? <FaFacebook /> : ""}
                            </span>
